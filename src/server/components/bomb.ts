...
export class Bomb extends BaseComponent<Attributes, ComponentInstance> {
	...
	public detonate() {
		...
		params.FilterDescendantsInstances = [Workspace.lobby, this.instance, impact, decoration];
		...
		for (const part of parts) {
			...
			if (humanoid) {
				if (!part.Parent?.IsA("Model")) continue;
				if (charactersAlreadyHit.includes(part.Parent)) continue;
